...
assert.match(homepage, /class="app-preview-banner(?:\s[^"]*)?"/, 'homepage has the Care Navigator preview banner');
assert.match(homepage, /care-navigator\/media\/care-navigator-family-banner\.png/, 'app banner uses the approved family planning cover');
assert.match(homepage, /class="app-preview-photo"/, 'app banner renders the approved cover as a responsive image');
assert.match(homepage, /class="app-preview-light-sweep"/, 'app banner includes the restrained light sweep effect');
assert.match(homepage, /@keyframes app-photo-breathe/, 'app banner gives the cover image subtle depth');
...
assert.match(homepage, /prefers-reduced-motion:[^{]+\{[\s\S]*\.app-preview-photo[\s\S]*animation:\s*none/, 'app banner honors reduced motion for the cover effects');
